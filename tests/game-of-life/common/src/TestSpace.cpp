...
TestSpace::TestSpace(int width, int height) : width_(width), height_(height) {}

void TestSpace::Initialize() {
  agents.clear();
  ...
  for (int y = 0; y < height_; ++y) {
    for (int x = 0; x < width_; ++x) {
      ...
      agents.push_back(TestAgent(x, y, alive));
    }
  }
}

std::vector<Space<TestAgent>::Region> TestSpace::SplitIntoRegions(
  ...
  for (int region_id = 0; region_id < num_regions; ++region_id) {
    ...
    // Add all agent indices for rows in this region
...
std::vector<TestAgent> TestSpace::GetRegionNeighbours(
  ...
  std::vector<TestAgent> neighbors;
  ...
  // Add agents from the row above this region (with toroidal wrap-around)
  int k_above_row = 0;
  if (kFirstRow > 0) {
  ...
  // Add agents from the row below this region (with toroidal wrap-around)
  int k_below_row = 0;
...
std::vector<ParallelABM::LocalSubRegion<TestAgent>> TestSpace::SplitLocalRegion(
    ParallelABM::LocalRegion<TestAgent>& region, int num_subregions) const {
  std::vector<ParallelABM::LocalSubRegion<TestAgent>> subregions;
  subregions.reserve(static_cast<size_t>(num_subregions));

  const std::vector<TestAgent>& agents = region.GetAgents();
...
TestAgent& TestSpace::GetAgentAt(int x, int y) {
  return agents[static_cast<size_t>(CoordToIndex(x, y))];
}

const TestAgent& TestSpace::GetAgentAt(int x, int y) const {
  return agents[static_cast<size_t>(CoordToIndex(x, y))];
}

int TestSpace::CoordToIndex(int x, int y) const noexcept {
  return y * width_ + x;
}

void TestSpace::Serialize(std::ostream& os, int step) const {
  // Build binary grid state (0 or 1 for each agent)
  std::vector<unsigned char> grid_state;
  grid_state.reserve(agents.size());

  for (const auto& agent : agents) {
    const unsigned char kAgentState = agent.alive ? 1 : 0;
    grid_state.push_back(kAgentState);
  }
